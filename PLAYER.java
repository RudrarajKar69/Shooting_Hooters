...
public class PLAYER extends JLabel implements ActionListener, MouseListener {

	int TIMEE=60,speed=5,temp,score2,score;
	Timer t;
	Random rnd = new Random();
	public int bsx = rnd.nextInt(30)+10, bsy = rnd.nextInt(30)+10, bx, by, mx, my, dx, dy;
	static int playerWidth, playerHeight,bWIDTH = 25, bHEIGHT = 15;
	...
	PLAYER(int time) {
		TIMEE=time;
		bullet = new ImageIcon("./res/BULLET.png");
		enemy = new ImageIcon("./res/Player-1.png");
		player = new ImageIcon("./res/KK-1.png.png");
		playerWidth= player.getImage().getWidth(null);
		playerHeight=player.getImage().getHeight(null);
		tttt=new TIMERI();
	...
	void CheckScore()
	{
		if(score>highscore) {
			highscore=score;
			File scoreF = new File(".\\src\\highscore.txt");
			if(!scoreF.exists())
			{
				try {
	...
	void COLLISION() {
		...
		eny = new int[enemy.getImage().getHeight(null)];
			...
			eny=new int[64];
		//System.out.println(enx.length+","+eny.length);
	...
	void Player(Graphics g) {
		if(player.getImage().getWidth(null)>0&&player.getImage().getHeight(null)>0)
			g.drawImage(player.getImage(), x, y, playerWidth, playerHeight, null);
			else
			{
				g.setColor(Color.white);
...
//		g.setColor(Color.red);
//		g.fillRect(this.bx, this.by, bWIDTH, bHEIGHT);
